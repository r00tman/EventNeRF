...
def bash_run(cmd):
    ...
    colmap_bin = '/home/viktor/soft/colmap/build/src/exe/colmap'
    cmd = colmap_bin + ' ' + cmd
    print('\nRunning cmd: ', cmd)
...
def run_sfm(img_dir, db_file, out_dir):
    ...
    cmd = ' mapper \
            ...
            --Mapper.ba_global_use_pba 1'.format(db_file, img_dir, out_dir)
...
def main(img_dir, out_dir, base_dir, run_mvs=False):
    ...
    # copy stuff over
    shutil.copy2(os.path.join(out_dir, 'posed_images/kai_cameras.json'), base_dir)
    shutil.copy2(os.path.join(out_dir, 'posed_images/kai_cameras_normalized.json'), base_dir)

    targetdir = os.path.join(base_dir, 'rgb')
    os.makedirs(targetdir)

    imgdir = os.path.join(out_dir, 'posed_images/images')
    for img in os.listdir(imgdir):
        shutil.copy2(os.path.join(imgdir, img), targetdir)
...
if __name__ == '__main__':
    ...
    base_dir = sys.argv[1]
    img_dir = os.path.join(base_dir, 'source')
    out_dir = os.path.join(base_dir, 'out')
    run_mvs = False
    main(img_dir, out_dir, base_dir, run_mvs=run_mvs)
